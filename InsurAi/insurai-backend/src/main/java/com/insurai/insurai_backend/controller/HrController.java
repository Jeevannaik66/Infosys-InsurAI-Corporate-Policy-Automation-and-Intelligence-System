...
public class HrController {
    ...
    public ResponseEntity<?> approveClaim(
        ...
        try {
            ...
            Claim updated = claimService.approveClaim(claimId, remarks); // <-- send email automatically
    ...
    public ResponseEntity<?> rejectClaim(
        ...
        try {
            validateHrToken(authHeader);
            String remarks = body.get("remarks");
            Claim updated = claimService.rejectClaim(claimId, remarks); // <-- send email automatically
            return ResponseEntity.ok(new ClaimDTO(updated));
        } catch (Exception e) {
...
public ResponseEntity<?> getFraudClaims(@RequestHeader(value = "Authorization") String authHeader) {
    try {
        ...
        // Fetch only claims assigned to this HR AND flagged for fraud
        List<Claim> claims = claimService.getClaimsByAssignedHr(hr.getId())
                .stream()
                .filter(Claim::isFraud)  // filter fraud-flagged claims
                .collect(Collectors.toList());
