...
const dashboardStats = useMemo(() => {
  ...
  const totalClaimAmount = claims.reduce((sum, claim) => {
    let amount = 0;
    if (claim.amount) {
      // Remove non-digit characters like ₹ and commas
      amount = parseFloat(String(claim.amount).replace(/[^0-9.-]+/g, "")) || 0;
    }
    return sum + amount;
  }, 0);
  ...
  const totalCoverage = policies.reduce((sum, policy) => {
    const coverageValue = Number(policy.coverageAmount || 0);
  ...
  return {
    claims: { 
      ...
      avg: avgClaimAmount // ✅ added average here
    },
