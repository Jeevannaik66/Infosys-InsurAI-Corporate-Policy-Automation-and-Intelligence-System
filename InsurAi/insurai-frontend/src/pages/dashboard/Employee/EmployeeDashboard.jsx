...
const fetchEmployeeData = async (token) => {
  try {
    ...
    const formattedPolicies = response.data.map((policy) => ({
      ...
      coverageAmount: policy.coverageAmount,        // numeric
      formattedCoverage: `$${policy.coverageAmount.toLocaleString('en-IN')}`, // string for display
...
const handleClaimSubmit = (e) => {
  ...
  if (!newClaim.type || !claimAmount || !newClaim.description || !selectedPolicyId) {
    showNotificationAlert("Please fill all claim fields.");
    return;
  }
  ...
  // ❌ Prevent submission if amount exceeds remaining coverage
  if (claimAmount > remainingCoverage) {
    showNotificationAlert(
      `Cannot submit claim. Entered amount exceeds remaining coverage (₹${remainingCoverage.toLocaleString("en-IN")})!`
    );
    return; // STOP here — claim will NOT be submitted
  }
...
const handleQuerySubmit = async (e) => {
  ...
  // Map selected policyId to policy name
...
<div className="dashboard-main">
  ...
  <aside className="dashboard-sidebar">
    <nav className="nav flex-column p-3">
      <a
        href="#"
        className={`nav-link mb-2 ${activeTab === "home" ? "active" : ""}`}
...
{activeTab === "notifications" && (
  <EmployeeNotification
    userDbId={Number(localStorage.getItem("id"))}  // numeric DB ID
    token={localStorage.getItem("token") || ""}
  />
)}
